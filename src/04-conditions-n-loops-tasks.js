...
function getCommonDirectoryPath(pathes) {
  ...
  function commonPrefix(arrays) {
    const firstDirs = arrays.map((a) => a[0]);
    if (!firstDirs.every((d) => d === firstDirs[0])) return [];
    const suffix = commonPrefix(arrays.map((a) => a.slice(1)));
    return [firstDirs[0]].concat(suffix);
  }

  const resultArray = commonPrefix(splitPathes);
  if (!resultArray.length) return '';
  if (resultArray.length === 1 && resultArray[0] === '') return '/';
  if (resultArray[0] === '') return `/${resultArray.slice(1).join('/')}/`;
...
function getMatrixProduct(m1, m2) {
  return m1.map((row) =>
    m2[0].map((_, j) => row.reduce((sum, val, i) => sum + val * m2[i][j], 0))
  );
}
